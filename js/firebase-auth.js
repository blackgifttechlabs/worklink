...
const PROVIDER_PROFILE_CACHE_TTL_MS = 2 * 60 * 1000;
const PROVIDER_LOOKUP_CACHE_TTL_MS = 30 * 60 * 1000;
const PROVIDER_LIST_CACHE_TTL_MS = 5 * 60 * 1000;
const JOB_LIST_CACHE_TTL_MS = 2 * 60 * 1000;
const USER_JOB_LIST_CACHE_TTL_MS = 2 * 60 * 1000;
...
let legacyMessageMigrationScheduled = false;
const userDocumentCache = new Map();
const userDocumentInFlight = new Map();
const providerProfileCache = new Map();
const providerProfileInFlight = new Map();
const providerLookupCache = new Map();
let providerListCache = null;
let providerListInFlight = null;
...
function getFreshCacheValue(cache, key, maxAgeMs) {
  if (!key) return null;
...
function getUserDocumentSessionKey(uid) {
  return `worklinkup_user_doc_${uid}`;
}

function getProviderLookupSessionKey(uid) {
  return `worklinkup_provider_lookup_${uid}`;
}

function getProviderListSessionKey() {
  return 'worklinkup_provider_list_cache';
}

function getJobListSessionKey() {
  return 'worklinkup_job_list_cache';
}

function getUserJobListSessionKey(uid) {
...
function cacheProviderLookup(uid, provinceSlug = '') {
  if (!uid || !provinceSlug) return;
  const entry = {
    value: { uid, provinceSlug },
    cachedAt: Date.now()
  };
  providerLookupCache.set(uid, entry);
  writeSessionJson(getProviderLookupSessionKey(uid), entry);
}

function getCachedProviderLookup(uid) {
  if (!uid) return null;
  const memoryValue = getFreshCacheValue(providerLookupCache, uid, PROVIDER_LOOKUP_CACHE_TTL_MS);
  if (memoryValue) return memoryValue;
  const sessionEntry = readSessionJson(getProviderLookupSessionKey(uid));
  if (!sessionEntry) return null;
  if (Date.now() - Number(sessionEntry.cachedAt || 0) > PROVIDER_LOOKUP_CACHE_TTL_MS) return null;
  providerLookupCache.set(uid, sessionEntry);
  return sessionEntry.value ?? null;
}
...
function cacheProviderList(providers = []) {
  ...
  writeSessionJson(getProviderListSessionKey(), entry);
}

function getCachedJobList() {
  if (jobListCache && Date.now() - Number(jobListCache.cachedAt || 0) <= JOB_LIST_CACHE_TTL_MS) {
    return jobListCache.value;
  }
  const sessionEntry = readSessionJson(getJobListSessionKey());
  if (!sessionEntry) return null;
  if (Date.now() - Number(sessionEntry.cachedAt || 0) > JOB_LIST_CACHE_TTL_MS) return null;
  jobListCache = sessionEntry;
  return sessionEntry.value;
}

function cacheJobList(jobs = []) {
  ...
  jobListCache = entry;
  writeSessionJson(getJobListSessionKey(), entry);
}

function clearJobListCache() {
  jobListCache = null;
  try {
    sessionStorage.removeItem(getJobListSessionKey());
  } catch (error) {
    // Ignore storage issues.
  }
}

function getCachedUserJobList(uid) {
  if (!uid) return null;
  const memoryValue = getFreshCacheValue(userJobListCache, uid, USER_JOB_LIST_CACHE_TTL_MS);
  if (memoryValue) return memoryValue;
  const sessionEntry = readSessionJson(getUserJobListSessionKey(uid));
  if (!sessionEntry) return null;
  if (Date.now() - Number(sessionEntry.cachedAt || 0) > USER_JOB_LIST_CACHE_TTL_MS) return null;
  userJobListCache.set(uid, sessionEntry);
  return sessionEntry.value ?? null;
}

function cacheUserJobList(uid, jobs = []) {
  if (!uid || !Array.isArray(jobs)) return;
  const entry = {
    value: jobs,
    cachedAt: Date.now()
  };
  userJobListCache.set(uid, entry);
  writeSessionJson(getUserJobListSessionKey(uid), entry);
}

function clearUserJobListCache(uid = '') {
  if (uid) {
    userJobListCache.delete(uid);
    userJobListInFlight.delete(uid);
    try {
      sessionStorage.removeItem(getUserJobListSessionKey(uid));
    } catch (error) {
      // Ignore storage issues.
    }
    return;
  }
  userJobListCache.clear();
  userJobListInFlight.clear();
}
...
async function getUserDocument(uid = auth.currentUser?.uid) {
  ...
  userDocumentInFlight.set(uid, request);
  try {
    return await request;
  } finally {
    userDocumentInFlight.delete(uid);
  }
}
...
async function getProviderProfileByUid(uid, knownProvinceSlug = '') {
  if (!uid) return null;
  const cachedProfile = getFreshCacheValue(providerProfileCache, uid, PROVIDER_PROFILE_CACHE_TTL_MS);
  if (cachedProfile) return cachedProfile;

  const pending = providerProfileInFlight.get(uid);
  if (pending) return pending;

  const request = (async () => {
    const provinceCandidates = [
      String(knownProvinceSlug || '').trim(),
      String(getCachedProviderLookup(uid)?.provinceSlug || '').trim()
    ].filter(Boolean).filter((value, index, list) => list.indexOf(value) === index);

    for (const provinceSlug of provinceCandidates) {
      const directSnapshot = await getDoc(doc(db, 'providers', provinceSlug, 'profiles', uid));
      if (!directSnapshot.exists()) continue;
      const directProfile = mapProviderProfile(directSnapshot);
      cacheProviderProfile(directProfile);
      return directProfile;
    }

    const userProvinceSlug = String((await getUserDocument(uid))?.providerProvinceSlug || '').trim();
    if (userProvinceSlug && !provinceCandidates.includes(userProvinceSlug)) {
      const directSnapshot = await getDoc(doc(db, 'providers', userProvinceSlug, 'profiles', uid));
    ...
    const groupedProfile = mapProviderProfile(grouped.docs[0]);
    cacheProviderProfile(groupedProfile);
    return groupedProfile;
  })();

  providerProfileInFlight.set(uid, request);
  try {
    return await request;
  } finally {
    providerProfileInFlight.delete(uid);
  }
}
...
async function listProviderReviews(uid, provinceSlug = '') {
  const providerProfile = await getProviderProfileByUid(uid, provinceSlug);
  if (!providerProfile) return [];
...
async function saveProviderProfile(profileInput = {}) {
  ...
  cacheProviderProfile(providerProfile);
  cacheUserDocument(account.uid, {
    ...(existingUserDoc || {}),
    uid: account.uid,
    name: displayName,
...
async function saveClientProfile(profileInput = {}) {
  ...
  cacheUserDocument(account.uid, {
    ...
    profileImageData: profilePayload.profileImageData,
    bannerImageData: profilePayload.bannerImageData,
    updatedAtMs: profilePayload.updatedAtMs
  });
...
async function createJobPost(payload = {}) {
  ...
  clearJobListCache();
  clearUserJobListCache(owner.uid);
...
async function listJobPosts(options = {}) {
  const forceRefresh = Boolean(options.forceRefresh);
  const includeApplicationCounts = Boolean(options.includeApplicationCounts);
  if (!forceRefresh && !includeApplicationCounts) {
    const cachedJobs = getCachedJobList();
    if (cachedJobs) return cachedJobs;
  }

  if (!includeApplicationCounts && jobListInFlight) return jobListInFlight;
  ...
  const loadJobs = async () => {
    ...
    const jobs = await Promise.all(snapshot.docs.map(async (docSnapshot) => {
      ...
      if (!includeApplicationCounts) return baseJob;
      const applicationsSnapshot = await getDocs(collection(db, JOBS_COLLECTION, docSnapshot.id, 'applications')).catch(() => null);
      return {
        ...baseJob,
        applicationCount: applicationsSnapshot?.size || baseJob.applicationCount || 0
      };
    }));

    const sortedJobs = jobs.sort((first, second) => Number(second.createdAtMs || 0) - Number(first.createdAtMs || 0));
    if (!includeApplicationCounts) cacheJobList(sortedJobs);
    return sortedJobs;
  };

  if (includeApplicationCounts) return loadJobs();

  jobListInFlight = loadJobs();
  try {
    return await jobListInFlight;
  } finally {
    jobListInFlight = null;
  }
}

async function listJobsForUser(uid = auth.currentUser?.uid, options = {}) {
  if (!uid) return [];
  const normalizedUid = String(uid || '').trim();
  const forceRefresh = Boolean(options.forceRefresh);
  if (!forceRefresh) {
    const cachedJobs = getCachedUserJobList(normalizedUid);
    if (cachedJobs) return cachedJobs;
    const inFlight = userJobListInFlight.get(normalizedUid);
    if (inFlight) return inFlight;
  }

  const loadJobs = async () => {
    const snapshot = await getDocs(query(collection(db, JOBS_COLLECTION), where('ownerUid', '==', normalizedUid)));
    const jobs = snapshot.docs
      .map((docSnapshot) => ({ id: docSnapshot.id, ...docSnapshot.data() }))
      .sort((first, second) => Number(second.createdAtMs || 0) - Number(first.createdAtMs || 0));
    cacheUserJobList(normalizedUid, jobs);
    return jobs;
  };

  const request = loadJobs();
  userJobListInFlight.set(normalizedUid, request);
  try {
    return await request;
  } finally {
    userJobListInFlight.delete(normalizedUid);
  }
}
...
async function deleteJobPost(jobId = '') {
  ...
  clearJobListCache();
  clearUserJobListCache(job.ownerUid || auth.currentUser.uid);
...
async function applyToJob(jobId = '', payload = {}) {
  ...
  const applicationPayload = {
    ...
    bidderName,
    bidderUsername: String(userDoc?.username || account.username || '').trim(),
...
async function updateJobApplicationStatus(jobId = '', applicationId = '', status = '') {
  ...
  await updateDoc(applicationRef, {
    ...
    inProgressAtMs: nextStatus === 'accepted' ? 0 : Number(application.inProgressAtMs || 0),
    inProgressByUid: nextStatus === 'accepted' ? '' : String(application.inProgressByUid || ''),
    completedAtMs: nextStatus === 'accepted' ? 0 : Number(application.completedAtMs || 0),
    completedByUid: nextStatus === 'accepted' ? '' : String(application.completedByUid || ''),
    reviewPendingAtMs: nextStatus === 'accepted' ? 0 : Number(application.reviewPendingAtMs || 0),
    reviewedAtMs: nextStatus === 'accepted' ? 0 : Number(application.reviewedAtMs || 0),
    rating: nextStatus === 'accepted' ? 0 : Number(application.rating || 0),
    reviewComment: nextStatus === 'accepted' ? '' : String(application.reviewComment || ''),
    review: nextStatus === 'accepted' ? null : application.review || null,
    statusChangedAtMs: now,
    statusChangedByUid: auth.currentUser.uid,
    updatedAtMs: now,
    updatedAt: serverTimestamp()
  ...
  if (nextStatus === 'accepted') {
    ...
    jobUpdates.inProgressAtMs = 0;
    jobUpdates.completedAtMs = 0;
    jobUpdates.reviewPendingAtMs = 0;
    jobUpdates.reviewedAtMs = 0;
...
async function listPendingJobReviews(uid = auth.currentUser?.uid) {
  ...
  const pendingReviews = await Promise.all(jobs.map(async (job) => {
    ...
    if (!applicationId || !completedAtMs) return null;

    const applicationRef = doc(db, JOBS_COLLECTION, job.id, 'applications', applicationId);
    const applicationSnapshot = await getDoc(applicationRef).catch(() => null);
    if (!applicationSnapshot?.exists()) return null;

    const application = { id: applicationSnapshot.id, ...applicationSnapshot.data() };
    const alreadyReviewed = Boolean(
      Number(application.reviewedAtMs || 0)
      || Number(application.review?.rating || 0)
    );
    ...
    return {
      ...
      completedAtMs: Number(application.completedAtMs || completedAtMs || 0)
    };
  }));

  return pendingReviews
    .filter(Boolean)
    .sort((first, second) => Number(second.completedAtMs || 0) - Number(first.completedAtMs || 0));
}
...
async function markJobStarted(jobId = '', applicationId = '') {
  ...
  if (String(job.acceptedApplicationUid || '').trim() !== String(applicationId || '').trim()) {
    throw new Error('This worker is not assigned to the selected job.');
  }
  const appRef = doc(db, JOBS_COLLECTION, jobId, 'applications', applicationId);
  const appSnap = await getDoc(appRef);
  if (!appSnap.exists()) throw new Error('Application not found.');
  const application = appSnap.data() || {};
  if (String(application.bidderUid || '').trim() !== auth.currentUser.uid) {
    throw new Error('Only the assigned worker can start this job.');
  }
  if (Number(application.completedAtMs || job.completedAtMs || 0)) {
    throw new Error('This job has already been finished.');
  }
  const now = Date.now();
...
async function markJobCompleted(jobId = '', applicationId = '') {
  ...
  const application = appSnap.data() || {};
  if (String(application.bidderUid || '').trim() !== auth.currentUser.uid) {
    throw new Error('Only the assigned worker can finish this job.');
  }
  if (Number(application.completedAtMs || job.completedAtMs || 0)) {
    throw new Error('This job has already been finished.');
  }
  if (!Number(application.inProgressAtMs || job.inProgressAtMs || 0)) {
    throw new Error('Start the job before marking it as finished.');
  }
  const now = Date.now();
  ...
  if (providerUid) {
    const completedJobs = (await listPlacedJobBids(providerUid).catch(() => []))
      .filter((item) => Number(item.completedAtMs || 0))
      .length;
...
async function submitJobReview(jobId = '', applicationId = '', review = {}) {
  if (!auth.currentUser) throw new Error('Please sign in first.');
  if (!jobId || !applicationId) throw new Error('Invalid job selection.');
  ...
  if (!jobSnap.exists()) throw new Error('Job not found.');
  const job = jobSnap.data() || {};
  if (String(job.ownerUid || '').trim() !== auth.currentUser.uid) {
    throw new Error('Only the job owner can rate this worker.');
  }
  if (String(job.acceptedApplicationUid || '').trim() !== String(applicationId || '').trim()) {
    throw new Error('This worker is not assigned to the selected job.');
  }
  ...
  const application = appSnap.data() || {};
  if (!Number(application.completedAtMs || job.completedAtMs || 0)) {
    throw new Error('The worker must finish the job before you can rate them.');
  }

  const rawReviewRating = Number(review.rating || 0);
  const reviewRating = Number.isFinite(rawReviewRating) ? Math.max(1, Math.min(5, rawReviewRating)) : 0;
  if (!reviewRating) {
  ...
  const completedAtMs = Number(application.completedAtMs || job.completedAtMs || now);
  const reviewPayload = {
    ...
    jobId,
    jobTitle: job.subcategory || job.category || 'Job',
    jobProviderName: job.ownerName || reviewerName || '',
    completedAtMs,
    createdAtMs: now
  };
  ...
  const providerUid = String(application.bidderUid || '').trim();
  if (providerUid) {
    const providerProfile = await getProviderProfileByUid(providerUid).catch(() => null);
    if (providerProfile) {
      const reviewsRef = collection(db, 'providers', providerProfile.provinceSlug || 'unknown', 'profiles', providerUid, 'reviews');
      const reviewRef = doc(db, 'providers', providerProfile.provinceSlug || 'unknown', 'profiles', providerUid, 'reviews', jobId);
      await setDoc(reviewRef, {
        ...reviewPayload,
        createdAt: serverTimestamp()
      }, { merge: true }).catch(() => {});

      const [allReviewsSnap, completedBids] = await Promise.all([
        getDocs(reviewsRef).catch(() => null),
        listPlacedJobBids(providerUid).catch(() => [])
      ]);
      const completedJobs = completedBids.filter((item) => Number(item.completedAtMs || 0)).length;
      if (allReviewsSnap) {
...
async function listProviders(options = {}) {
  const forceRefresh = Boolean(options.forceRefresh);
  if (!forceRefresh) {
    const cachedProviders = getCachedProviderList();
    if (cachedProviders) return cachedProviders;
  }

  if (providerListInFlight) return providerListInFlight;
  ...
  try {
    return await providerListInFlight;
  } finally {
    providerListInFlight = null;
  }
}
...
function scheduleRealtimeMessagesMigration() {
  if (legacyMessageMigrationPromise || legacyMessageMigrationScheduled) return;
  legacyMessageMigrationScheduled = true;
  const defer = typeof window !== 'undefined' && typeof window.setTimeout === 'function'
    ? window.setTimeout.bind(window)
    : setTimeout;
  defer(() => {
    ensureRealtimeMessagesMigrated().catch(() => {});
  }, 0);
}
...
async function markConversationViewed(peerUid, knownMessages = null) {
  if (!auth.currentUser || !peerUid) return { updated: 0 };
  scheduleRealtimeMessagesMigration();
  const conversationId = createConversationId(auth.currentUser.uid, peerUid);
  const sourceMessages = Array.isArray(knownMessages)
    ? knownMessages
    : await get(ref(realtimeDb, `${MESSAGE_THREADS_PATH}/${conversationId}`)).then((snapshot) => (
      snapshot.exists()
