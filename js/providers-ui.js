(function providerUiBootstrap() {
  ...
  function getProviderJobStartedAt(record = {}) {
    const job = record.job || {};
    return Number(record.inProgressAtMs || job.inProgressAtMs || record.acceptedAtMs || 0);
  }
  ...
  async function renderProviderProfilePage() {
    ...
    async function refreshProfile() {
      ...
      if (servicesContainer && servicesHost && freshProvider.services) {
        const serviceItems = Array.isArray(freshProvider.services) ? freshProvider.services : [];
        if (serviceItems.length > 0) {
          servicesContainer.hidden = false;
          servicesHost.innerHTML = serviceItems.map((service) => `
            <span class="provider-service-tag">${escapeHtml(service)}</span>
          `).join('');
        }
      }
                ...
                <div class="provider-job-worked-card">
                  ...
                  ${job.rating ? `
                    <div class="provider-job-worked-rating">
                      <span class="rating-stars">${job.rating} ★</span>
                    </div>
                  ` : ''}
                  ${job.comment ? `
