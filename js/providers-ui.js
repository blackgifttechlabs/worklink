(function providerUiBootstrap() {
  ...
  function normalizeProviderServiceValue(value) {
    if (typeof value === 'string') return value.trim();
    if (!value || typeof value !== 'object') return '';
    const nestedValue = value.service
      || value.specialty
      || value.name
      || value.label
      || value.category
  ...
  function normalizeProviderServiceItem(item = {}) {
    ...
    const rawService = normalizeProviderServiceValue(item.service)
      || normalizeProviderServiceValue(item.specialty)
      || normalizeProviderServiceValue(item.name)
      || normalizeProviderServiceValue(item.label)
      || normalizeProviderServiceValue(item.category);
    const service = rawService || normalizeProviderServiceValue(item.primaryCategory);
  ...
  function formatProviderCurrency(value) {
    ...
    if (!Number.isFinite(amount) || amount <= 0) return 'Budget not shared';
    return `US$${amount.toLocaleString(undefined, { maximumFractionDigits: 0 })}`;
  }

  function formatProviderJobDate(timestamp, fallback = 'Date not recorded') {
  ...
  function getProviderJobTitle(record = {}) {
    const job = record.job || {};
    return job.subcategory || record.jobSubcategory || record.jobTitle || job.category || record.jobCategory || 'Job';
  }

  function getProviderJobDescription(record = {}) {
    const job = record.job || {};
    return job.description || record.comment || record.bidderMessage || 'No description provided.';
  }
  ...
  function buildProviderRatingStarsMarkup(rating = 0) {
    const normalizedRating = Math.max(0, Math.min(5, Math.round(Number(rating || 0))));
    return `
      <span class="provider-rating-stars" aria-label="${escapeHtml(String(normalizedRating))} out of 5 stars">
  ...
  function getProviderJobCompletedAt(record = {}) {
    const job = record.job || {};
    return Number(record.completedAtMs || record.review?.createdAtMs || job.completedAtMs || 0);
  }

  function getProviderJobStartedAt(record = {}) {
    const job = record.job || {};
    return Number(record.inProgressAtMs || job.inProgressAtMs || 0);
  }

  function getProviderJobStatus(record = {}) {
    const status = String(record.status || '').trim().toLowerCase();
    if (status === 'completed' || getProviderJobCompletedAt(record)) return 'completed';
    if (status === 'rejected') return 'rejected';
    if (getProviderJobStartedAt(record) && status === 'accepted') return 'in progress';
    return status || 'pending';
  }

  function buildProviderJobCard(record = {}, options = {}) {
    const job = record.job || {};
    const status = getProviderJobStatus(record);
    const rating = getProviderJobRating(record);
    const comment = getProviderJobComment(record);
    const date = options.date || getProviderJobCompletedAt(record) || getProviderJobStartedAt(record) || record.createdAtMs || job.createdAtMs;
    const showPrice = Boolean(options.showPrice);
    const showClient = Boolean(options.showClient);
    const showDescription = options.showDescription !== false;
    const showReview = Boolean(options.showReview);
    const cssClass = options.cssClass || 'provider-bid-job-card';
    const statusClass = status.replace(/\s+/g, '-');
    const actionsMarkup = String(options.actionsMarkup || '').trim();
    return `
      <article class="provider-gallery-card ${escapeHtml(cssClass)}">
        <div class="provider-job-status-pill is-${escapeHtml(statusClass)}">${escapeHtml(status)}</div>
        <div class="provider-gallery-card-copy">
          <strong>${escapeHtml(getProviderJobTitle(record))}</strong>
          ${showDescription ? `<p>${escapeHtml(getProviderJobDescription(record))}</p>` : ''}
          <div class="provider-job-meta">
            <span><i class="fa-regular fa-calendar"></i>${escapeHtml(formatProviderJobDate(date))}</span>
            ${rating ? `<span class="provider-job-rating"><i class="fa-solid fa-star"></i>${escapeHtml(String(rating))}</span>` : ''}
            ${showPrice ? `<span><i class="fa-solid fa-tag"></i>${escapeHtml(formatProviderCurrency(record.proposedBudget || job.acceptedApplicationBudget || job.budget))}</span>` : ''}
            ${showClient && job.ownerName ? `<span><i class="fa-regular fa-user"></i>${escapeHtml(job.ownerName)}</span>` : ''}
          </div>
          ${showReview ? `
            <div class="provider-job-review">
              <div class="provider-job-review-head">
                ${rating ? buildProviderRatingStarsMarkup(rating) : '<span class="provider-job-review-empty">No rating yet</span>'}
              </div>
              ${comment ? `<p class="provider-job-review-comment">${escapeHtml(comment)}</p>` : '<p class="provider-job-review-comment is-empty">No comment provided.</p>'}
            </div>
          ` : ''}
          ${actionsMarkup ? `<div class="provider-job-actions">${actionsMarkup}</div>` : ''}
        </div>
      </article>
    `;
  }

  function buildProviderCurrentJobCard(record = {}) {
    const job = record.job || {};
  ...
  function createCircleCardsMarkup(base, isLoop = false, options = {}) {
    const isClone = Boolean(options.isClone);
    const categories = SPECIALIST_CATEGORIES.filter((category) => String(category.image || '').trim());
    const cards = categories.map((category) => `
      <a href="${buildSearchResultsHref(category.label, { base, category: category.label })}" class="category-circle specialist-category-chip" data-category-chip="${category.label}"${isClone ? ' aria-hidden="true" tabindex="-1" data-loop-clone="1"' : ''}>
    ...
    return isLoop ? `${cards}${createCircleCardsMarkup(base, false, { isClone: true })}` : cards;
  }
  ...
  const SPECIALISTS_PROVIDER_CACHE_KEY = 'worklinkup_specialists_provider_cache';
  const SPECIALISTS_PROVIDER_CACHE_TTL_MS = 5 * 60 * 1000;

  function readCachedSpecialistsProviders() {
    try {
      const raw = sessionStorage.getItem(SPECIALISTS_PROVIDER_CACHE_KEY);
      if (!raw) return [];
      const entry = JSON.parse(raw);
      if (Date.now() - Number(entry.cachedAt || 0) > SPECIALISTS_PROVIDER_CACHE_TTL_MS) return [];
      return Array.isArray(entry.providers) ? entry.providers.map(normalizeProvider) : [];
    } catch (error) {
      return [];
    }
  }

  function writeCachedSpecialistsProviders(providers = []) {
    try {
      if (!Array.isArray(providers) || !providers.length) return;
      sessionStorage.setItem(SPECIALISTS_PROVIDER_CACHE_KEY, JSON.stringify({
        cachedAt: Date.now(),
        providers
      }));
    } catch (error) {
  ...
  async function getProviders(options = {}) {
    ...
    try {
      const remoteProviders = await authHelper.listProviders(options);
      const normalizedProviders = remoteProviders.map(normalizeProvider);
      writeCachedSpecialistsProviders(normalizedProviders);
  ...
  function renderProviderCards(host, providers) {
    ...
    host.innerHTML = providers.map((provider, index) => {
      ...
      return `
      <article class="specialist-card provider-showcase-card wl-showcase-card wl-showcase-card--split is-viewport-card" data-card-index="${index}">
        <a class="wl-showcase-card__media wl-showcase-card__media--split" href="${escapeHtml(provider.profileUrl)}" aria-label="View ${escapeHtml(provider.displayName)} profile">
          <img class="wl-showcase-card__image" src="${escapeHtml(heroImage)}" alt="${escapeHtml(provider.displayName)} profile image" loading="lazy" decoding="async" />
        </a>
        <div class="wl-showcase-card__content wl-showcase-card__content--split">
          <div class="wl-showcase-card__eyebrow-row">
            <span class="wl-showcase-card__eyebrow">${escapeHtml(provider.primaryCategory || 'Provider')}</span>
            <strong class="wl-showcase-card__price">${escapeHtml(ratingValue)} ★</strong>
          </div>
          <h3 class="wl-showcase-card__title">${escapeHtml(provider.displayName)}</h3>
          <p class="wl-showcase-card__subtitle">${escapeHtml(serviceLabel)}</p>
          ...
          <p class="wl-showcase-card__copy">${shortBio}</p>
          <div class="wl-showcase-card__actions wl-showcase-card__actions--split specialist-actions">
            <a href="${escapeHtml(provider.profileUrl)}" class="wl-showcase-card__action wl-showcase-card__action--split specialists-view-btn" aria-label="View ${escapeHtml(provider.displayName)} profile">
              <span>View Profile</span>
              <i class="fa-solid fa-arrow-right"></i>
            </a>
            <a href="${escapeHtml(whatsappHref)}" class="wl-showcase-card__icon-action provider-contact-btn whatsapp-btn" target="_blank" rel="noreferrer" aria-label="Chat with ${escapeHtml(provider.displayName)} on WhatsApp">
  ...
  function clearSpecialistCardViewportMotion(host) {
    if (!(host instanceof HTMLElement)) return;
    if (host._specialistCardObserver instanceof IntersectionObserver) {
      host._specialistCardObserver.disconnect();
    }
    host._specialistCardObserver = null;
  }

  function initSpecialistCardViewportMotion(host) {
    if (!(host instanceof HTMLElement)) return;
    clearSpecialistCardViewportMotion(host);

    const cards = Array.from(host.querySelectorAll('.specialist-card.is-viewport-card'));
    ...
    cards.forEach((card, index) => {
      card.style.setProperty('--specialist-card-shift', index % 2 === 0 ? '-24px' : '24px');
    });
    ...
    host._specialistCardObserver = observer;
  }
  ...
  function buildSpecialistCardSkeletons(count = 4) {
    return Array.from({ length: count }, () => `
      <article class="specialist-card provider-showcase-card provider-showcase-card--skeleton specialist-card-skeleton-card" aria-hidden="true">
        <div class="provider-showcase-card__skeleton-media provider-skeleton-block"></div>
        <div class="wl-showcase-card__content wl-showcase-card__content--split provider-showcase-card__skeleton-content">
          <div class="wl-showcase-card__eyebrow-row provider-showcase-card__skeleton-row">
            <span class="provider-showcase-card__skeleton-pill provider-skeleton-line"></span>
            <span class="provider-showcase-card__skeleton-pill provider-showcase-card__skeleton-pill--small provider-skeleton-line"></span>
          </div>
          <span class="provider-showcase-card__skeleton-title provider-skeleton-line"></span>
          <span class="provider-showcase-card__skeleton-subtitle provider-skeleton-line"></span>
          <div class="provider-showcase-card__skeleton-details">
            <span class="provider-showcase-card__skeleton-detail provider-skeleton-line"></span>
            <span class="provider-showcase-card__skeleton-detail provider-showcase-card__skeleton-detail--short provider-skeleton-line"></span>
            <span class="provider-showcase-card__skeleton-detail provider-skeleton-line"></span>
          </div>
          <div class="provider-showcase-card__skeleton-copy">
            <span class="provider-showcase-card__skeleton-line provider-skeleton-line"></span>
            <span class="provider-showcase-card__skeleton-line provider-showcase-card__skeleton-line--short provider-skeleton-line"></span>
          </div>
          <div class="wl-showcase-card__actions wl-showcase-card__actions--split specialist-actions provider-showcase-card__skeleton-actions">
            <span class="provider-showcase-card__skeleton-action provider-skeleton-block"></span>
            <span class="provider-showcase-card__skeleton-icon provider-skeleton-block"></span>
          </div>
        </div>
      </article>
    `).join('');
  ...
  async function renderSpecialistsPage() {
    ...
    const searchShell = page.querySelector('.specialists-search-shell');
    const searchPanel = page.querySelector('[data-specialists-search-panel]');
    ...
    const searchToggleBtn = page.querySelector('[data-toggle-specialists-search]');
    ...
    let isSearchOpen = Boolean(searchQueryParam);
    const state = {
    ...
    function syncSearchPanel() {
      if (searchShell instanceof HTMLElement) {
        searchShell.classList.toggle('is-search-open', isSearchOpen);
      }
      if (searchToggleBtn instanceof HTMLElement) {
        const hasSearchValue = searchInput instanceof HTMLInputElement && Boolean(searchInput.value.trim());
        searchToggleBtn.classList.toggle('is-active', isSearchOpen || hasSearchValue);
        searchToggleBtn.setAttribute('aria-expanded', String(isSearchOpen));
        searchToggleBtn.setAttribute('aria-label', isSearchOpen ? 'Close search' : 'Open search');
      }
      if (searchPanel instanceof HTMLElement) {
        searchPanel.setAttribute('aria-hidden', String(!isSearchOpen));
      }
    }
    ...
    async function update({ showSkeleton = false } = {}) {
      ...
      if (showSkeleton && resultsHost) {
        clearSpecialistCardViewportMotion(resultsHost);
        resultsHost.innerHTML = buildSpecialistCardSkeletons(Math.max(4, Math.min(filtered.length || 4, 6)));
      }

      await new Promise((resolve) => window.setTimeout(resolve, showSkeleton ? 120 : 0));
      if (requestId !== renderRequestId) return;

      if (!filtered.length) {
        if (resultsHost) {
          clearSpecialistCardViewportMotion(resultsHost);
          resultsHost.innerHTML = buildEmptyStateMarkup(getActiveSearchLabel());
        }
      } else {
        renderProviderCards(resultsHost, filtered);
        initSpecialistCardViewportMotion(resultsHost);
      }
    ...
    searchInput?.addEventListener('input', syncSearchPanel);

    searchToggleBtn?.addEventListener('click', () => {
      isSearchOpen = !isSearchOpen;
      syncSearchPanel();
      if (isSearchOpen && searchInput instanceof HTMLInputElement) {
    ...
    filterBtn?.addEventListener('click', () => {
      if (!mobileQuery.matches || !mobileSheet) return;
      mobileSheet.hidden = false;
    });

    ratingSheetBtn?.addEventListener('click', () => {
      if (!mobileQuery.matches || !mobileRatingSheet) return;
    ...
    mobileQuery.addEventListener('change', (event) => {
      renderCategoryRail();
      if (!event.matches) closeMobileSheets();
    });

    const cachedProviders = readCachedSpecialistsProviders();
    if (cachedProviders.length) {
      providers.splice(0, providers.length, ...cachedProviders);
      await update({ showSkeleton: false });
    ...
    if (!cachedProviders.length) {
      ...
      if (resultsHost) resultsHost.innerHTML = buildSpecialistCardSkeletons(4);
    }

    try {
      const remoteProviders = cachedProviders.length ? [] : await getProviders();
      if (remoteProviders.length) {
        providers.splice(0, providers.length, ...remoteProviders);
        await update({ showSkeleton: true });
      }
    } finally {
      if (page instanceof HTMLElement) page.classList.remove('is-loading');
    }
  ...
  function buildProviderWorkEmptyState(message = 'No previous work yet. Completed jobs will appear here.', icon = 'fa-regular fa-image') {
    return `
  ...
  async function renderProviderProfilePage() {
    ...
    const currentGrid = page.querySelector('[data-provider-current-grid]');
    const workedGrid = page.querySelector('[data-provider-worked-grid]');
    const bidsGrid = page.querySelector('[data-provider-bids-grid]');
    const currentCountBadge = page.querySelector('[data-provider-current-count]');
    ...
    async function refreshProfile() {
      ...
      if (postGrid instanceof HTMLElement) {
        postGrid.innerHTML = buildProviderWorkSkeleton(4);
      }

      const postsPromise = getPostsForProvider(uid, provinceSlug).catch(() => []);
      const freshProvider = await getProviderByIdentity(uid, provinceSlug);
      if (!freshProvider) return;

      const providerServices = getProviderServiceList(freshProvider);
      const serviceLabel = getServiceListLabel(providerServices)
        || normalizeProviderServiceValue(freshProvider.specialty)
        || normalizeProviderServiceValue(freshProvider.primaryCategory)
        || 'Specialist';
      const primaryCategoryLabel = providerServices[0]?.category || normalizeProviderServiceValue(freshProvider.primaryCategory);
      const workLabelParts = [
        normalizeProviderServiceValue(freshProvider.title) || serviceLabel,
        primaryCategoryLabel
      ].filter(Boolean);
      const workLabel = workLabelParts.filter((item, index) => workLabelParts.indexOf(item) === index).join(' • ') || 'Specialist';
      ...
      if (editSection instanceof HTMLElement) {
        editSection.hidden = !isOwner;
      }
      ...
      const ownerOnlyTabs = new Set(['bids']);
      ...
      function setActiveGalleryTab(tabName = 'previous') {
        ...
        galleryContents.forEach((content) => {
          const isActive = content.getAttribute('data-gallery-content') === nextTab;
          content.hidden = !isActive;
          content.classList.toggle('is-active-gallery', isActive);
        });

        activeGalleryTab = nextTab;
      }

      galleryTabs.forEach((tab) => {
        const tabName = tab.getAttribute('data-gallery-tab') || '';
        tab.setAttribute('role', 'tab');
        if (ownerOnlyTabs.has(tabName)) {
          tab.hidden = !isOwner;
        }
      });

      galleryContents.forEach((content) => {
        const contentName = content.getAttribute('data-gallery-content') || '';
        content.setAttribute('role', 'tabpanel');
        content.hidden = contentName !== activeGalleryTab || (ownerOnlyTabs.has(contentName) && !isOwner);
        content.classList.toggle('is-active-gallery', contentName === activeGalleryTab);
      });
      
      galleryTabs.forEach((tab) => {
        tab.addEventListener('click', () => {
          const tabName = tab.getAttribute('data-gallery-tab');
          if (!tabName || (ownerOnlyTabs.has(tabName) && !isOwner)) return;
          setActiveGalleryTab(tabName);
        });
      });
          ...
          openProviderDialog(`
            <div class="provider-ratings-modal">
              ...
              ${ratings.length ? ratings.map((r) => `
                <div class="provider-rating-item">
                  <div class="provider-rating-header">
                    <div class="provider-rating-job">${escapeHtml(r.jobTitle || 'Job')}</div>
                    <div class="provider-rating-value">${r.rating || 0} ★</div>
                  </div>
                  <div class="provider-rating-provider">From: ${escapeHtml(r.jobProviderName || 'Client')}</div>
                  <div class="provider-rating-comment">${r.comment ? escapeHtml(r.comment) : '<span class="provider-rating-no-comment">No comment provided</span>'}</div>
                </div>
              `).join('') : '<p>No ratings yet. Complete jobs to receive ratings!</p>'}
            </div>
          `);
        });
      }

      // Setup services display
      const servicesContainer = page.querySelector('[data-provider-services-container]');
      const servicesHost = page.querySelector('[data-provider-services]');
      if (servicesContainer && servicesHost) {
        const serviceItems = getProviderServiceList(freshProvider);
        if (serviceItems.length > 0) {
          servicesContainer.hidden = false;
          servicesHost.innerHTML = serviceItems.map((service) => `
            <span class="provider-service-tag">${escapeHtml(service.service || service.category || 'Service')}</span>
          `).join('');
        } else {
      ...
      // Setup skills modal trigger
      const skillsModal = page.querySelector('[data-provider-skills-modal]');
      if (skillsModal) {
      ...
      // Setup jobs done modal trigger
      const jobsDoneButton = page.querySelector('[data-provider-jobs-modal]');
      if (jobsDoneButton) {
        jobsDoneButton.addEventListener('click', () => {
          const completedJobs = freshProvider.completedJobsDetails || [];
          openProviderDialog(`
            <div class="provider-jobs-modal">
              <h2>Jobs completed</h2>
              ${completedJobs.length ? completedJobs.map((job) => `
                <div class="provider-job-worked-card">
                  <div class="provider-job-worked-title">${escapeHtml(job.jobTitle || 'Job')}</div>
                  <div class="provider-job-worked-provider">Posted by: ${escapeHtml(job.jobProviderName || 'Client')}</div>
                  ${Number(job.rating || 0) ? `
                    <div class="provider-job-worked-rating">
                      ${buildProviderRatingStarsMarkup(job.rating)}
                    </div>
                  ` : '<div class="provider-job-worked-rating"><span class="provider-job-review-empty">No rating yet</span></div>'}
                  ${job.comment ? `
                    <div class="provider-job-worked-comment">${escapeHtml(job.comment)}</div>
                  ` : '<div class="provider-job-worked-comment no-comment">No feedback provided</div>'}
                </div>
              `).join('') : '<p>No completed jobs yet. Start working on jobs to see them here!</p>'}
            </div>
          `);
        });
      }

      // Setup current jobs, worked jobs, and placed bids tabs
      const currentJobsGrid = currentGrid;
      const workedJobsGrid = workedGrid;
      const placedBidsGrid = bidsGrid;
      
      if (currentJobsGrid || workedJobsGrid || placedBidsGrid) {
        const authHelper = await waitForAuthHelper();
        if (authHelper) {
          try {
            ...
            let liveProviderReviews = typeof authHelper.listProviderReviews === 'function'
              ...
              : [];
            let livePlacedBids = typeof authHelper.listPlacedJobBids === 'function'
            ...
            async function renderProviderJobTabs() {
              ...
              if (currentJobsGrid) {
                currentJobsGrid.innerHTML = currentJobs.length
                  ? currentJobs.map((record) => isOwner
                    ? buildProviderCurrentJobCard(record)
                    : buildProviderJobCard(record, {
                      cssClass: 'provider-current-job-card',
                      showPrice: true,
                      showClient: true,
                      date: getProviderJobStartedAt(record) || record.acceptedAtMs || record.createdAtMs
                    })
                  ).join('')
                  : buildProviderWorkEmptyState('No current jobs yet.', 'fa-regular fa-briefcase');

                currentJobsGrid.querySelectorAll('[data-provider-job-start]').forEach((button) => {
                  button.addEventListener('click', async () => {
                    const jobId = button.getAttribute('data-provider-job-start') || '';
                    const applicationId = button.getAttribute('data-provider-app-id') || '';
                    button.disabled = true;
                    try {
                      await authHelper.markJobStarted(jobId, applicationId);
                      showSuccessToast('Job started');
                      activeGalleryTab = 'current';
                      window.dispatchEvent(new CustomEvent('worklinkup-job-updated'));
                      await renderProviderJobTabs();
                    } catch (error) {
                      window.alert(error.message || 'Could not start this job.');
                      button.disabled = false;
                    }
                  });
                });

                currentJobsGrid.querySelectorAll('[data-provider-job-finish]').forEach((button) => {
                  button.addEventListener('click', async () => {
                    const jobId = button.getAttribute('data-provider-job-finish') || '';
                    const applicationId = button.getAttribute('data-provider-app-id') || '';
                    button.disabled = true;
                    try {
                      await authHelper.markJobCompleted(jobId, applicationId);
                      showSuccessToast('Job finished');
                      activeGalleryTab = 'worked';
                      window.dispatchEvent(new CustomEvent('worklinkup-job-updated'));
                      await renderProviderJobTabs();
                    } catch (error) {
                      window.alert(error.message || 'Could not finish this job.');
                      button.disabled = false;
                    }
                  });
                });
              ...
              } else if (placedBidsGrid) {
                placedBidsGrid.innerHTML = '';
              }
              setActiveGalleryTab(activeGalleryTab);
  ...
  async function renderPostsPage() {
    ...
    const dropzone = page.querySelector('[data-provider-post-dropzone]');
    const captionWrap = page.querySelector('[data-provider-post-caption-wrap]');
    const captionInput = page.querySelector('[data-provider-post-caption]');
    const submitButton = page.querySelector('[data-provider-post-submit]');
    let previewImageData = '';

    function syncComposerState() {
      const hasImage = Boolean(previewImageData);
      if (preview instanceof HTMLElement) preview.hidden = !hasImage;
      if (captionWrap instanceof HTMLElement) captionWrap.hidden = !hasImage;
      if (submitButton instanceof HTMLButtonElement) submitButton.disabled = !hasImage;
    ...
    function resetComposer() {
      ...
      if (captionWrap instanceof HTMLElement) captionWrap.hidden = true;
    ...
    function openProviderPostCropModal(sourceDataUrl = '') {
      return new Promise((resolve, reject) => {
        const shell = document.createElement('div');
        shell.className = 'provider-post-crop-shell';
        shell.innerHTML = `
          <div class="provider-post-crop-panel" role="dialog" aria-modal="true" aria-label="Crop post image">
            <div class="provider-post-crop-head">
              <div>
                <span>Crop image</span>
                <h3>Drag the edges or corners</h3>
              </div>
            ...
            <div class="provider-post-crop-stage">
              <div class="provider-post-crop-frame" data-provider-crop-frame>
                <img src="${escapeHtml(sourceDataUrl)}" alt="Selected work preview" data-provider-crop-image />
        ...
        function getBounds() {
          return {
            width: Math.max(1, image?.clientWidth || frame?.clientWidth || 1),
            height: Math.max(1, image?.clientHeight || frame?.clientHeight || 1)
          };
        }
        ...
        function initializeCrop() {
          ...
          crop.width = Math.max(minSize, Math.round(bounds.width * 0.8));
          crop.height = Math.max(minSize, Math.round(bounds.height * 0.8));
        ...
        function startDrag(event, handle = 'move') {
          event.preventDefault();
          dragState = {
            handle,
        ...
        function onDrag(event) {
          ...
          if (handle === 'move') {
            ...
            if (handle.includes('e')) {
              next.width = Math.min(Math.max(minSize, original.width + dx), bounds.width - original.x);
            }
            ...
            if (handle.includes('s')) {
              next.height = Math.min(Math.max(minSize, original.height + dy), bounds.height - original.y);
            }
          }
        ...
        box?.addEventListener('mousedown', (event) => {
          if (event.target?.hasAttribute?.('data-crop-handle')) return;
        ...
        shell.querySelectorAll('[data-crop-handle]').forEach((handleButton) => {
          const handle = handleButton.getAttribute('data-crop-handle') || 'se';
          handleButton.addEventListener('mousedown', (event) => startDrag(event, handle));
          handleButton.addEventListener('touchstart', (event) => startDrag(event, handle), { passive: false });
        });

        shell.querySelectorAll('[data-provider-crop-cancel]').forEach((button) => {
          button.addEventListener('click', () => {
            closeModal();
        ...
        shell.querySelector('[data-provider-crop-finish]')?.addEventListener('click', async () => {
          try {
            const bounds = getBounds();
            const cropped = await cropImageDataUrl(sourceDataUrl, crop, bounds);
            closeModal();
            resolve(cropped);
          } catch (error) {
            closeModal();
    ...
    async function refreshPosts() {
      ...
      feedHost.innerHTML = posts.length
        ? posts.map((post, index) => `
          <article class="provider-post-feed-card">
            <button type="button" class="provider-post-gallery-tile" data-provider-post-view="${index}" aria-label="Open posted work">
      ...
      feedHost.querySelectorAll('[data-provider-post-view]').forEach((button) => {
        button.addEventListener('click', () => {
          const index = Number(button.getAttribute('data-provider-post-view') || -1);
          const post = posts[index];
          if (post) openProviderPostViewer(post);
        });
      });
    }

    function openProviderPostViewer(post = {}) {
      const imageSrc = resolveMediaSrc(post.imageData, 'images/sections/findme.avif');
      const caption = String(post.caption || '').trim();
      const shell = document.createElement('div');
      shell.className = 'provider-post-viewer-shell';
      shell.innerHTML = `
        <div class="provider-post-viewer-panel" role="dialog" aria-modal="true" aria-label="Posted work preview">
          <button type="button" class="provider-post-viewer-close" data-provider-post-viewer-close aria-label="Close post preview">x</button>
          <img src="${escapeHtml(imageSrc)}" alt="Posted work" />
          <div class="provider-post-viewer-caption">
            ${caption ? `<p>${escapeHtml(caption)}</p>` : `<p class="is-empty">No caption added.</p>`}
          </div>
        </div>
      `;
      ...
      shell.querySelectorAll('[data-provider-post-viewer-close]').forEach((button) => {
        button.addEventListener('click', closeViewer);
      });
      shell.addEventListener('click', (event) => {
        if (event.target === shell) closeViewer();
      });
    }
    ...
    async function handleSelectedPostImage(file) {
      ...
      try {
        ...
        previewImageData = await openProviderPostCropModal(sourceDataUrl);
    ...
    fileInput?.addEventListener('change', async () => {
      await handleSelectedPostImage(fileInput.files?.[0]);
    });
    ...
    dropzone?.addEventListener('drop', async (event) => {
      event.preventDefault();
      dropzone.classList.remove('is-dragover');
      await handleSelectedPostImage(event.dataTransfer?.files?.[0]);
    });
    ...
    form?.addEventListener('submit', async (event) => {
      ...
      try {
        ...
        if (submitBtn instanceof HTMLButtonElement) submitBtn.disabled = !previewImageData;
      }
    });
  ...
  function buildMessageSearchTextMarkup(text, query = '') {
    ...
    return rawText
      ...
      .map((part) => (
        part.toLowerCase() === search.toLowerCase()
          ? `<mark class="message-search-highlight">${escapeHtml(part)}</mark>`
          : escapeHtml(part)
      ))
      .join('');
  }
  ...
  function buildThreadMessagesMarkup(messages, accountUid, searchQuery = '') {
    let previousDayKey = '';
  ...
  async function renderMessagesPage() {
    ...
    let authSessionReady = Promise.resolve(null);
    let shouldRefreshAfterAuthSession = false;
    if (typeof authHelper.waitForAuthSession === 'function') {
      shouldRefreshAfterAuthSession = true;
      authSessionReady = authHelper.waitForAuthSession(account.uid, 2500).catch(() => null);
      await Promise.race([
        authSessionReady,
      ...
      <div class="messages-layout" data-messages-layout>
        ...
        <section class="messages-thread">
          ...
          <label class="messages-thread-search" data-thread-search-shell hidden>
            <i class="fa-solid fa-magnifying-glass"></i>
            <input type="search" placeholder="Search this chat" data-thread-message-search />
            <button type="button" data-thread-search-clear aria-label="Clear chat search">
    ...
    const threadSearchShell = page.querySelector('[data-thread-search-shell]');
    const threadSearchInput = page.querySelector('[data-thread-message-search]');
    const threadSearchClear = page.querySelector('[data-thread-search-clear]');
    ...
    let conversationProfileHydrationId = 0;
    ...
    if (chatList) {
      chatList.innerHTML = Array.from({ length: 5 }).map(() => `
        <div class="messages-chat-item-skeleton" aria-hidden="true">
    ...
    if (threadBody) {
      threadBody.innerHTML = `
        <div class="messages-empty">
          <div>
            <h2>Select a chat</h2>
            <p>Your conversations and search results will appear on the left.</p>
          </div>
        </div>
      `;
    }
    ...
    function setThreadSearchOpen(isOpen) {
      if (!(threadSearchShell instanceof HTMLElement)) return;
      const canSearch = Boolean(state.activePeerUid);
      const shouldOpen = Boolean(isOpen && canSearch);
      threadSearchShell.hidden = !shouldOpen;
      page.classList.toggle('is-thread-search-open', shouldOpen);
      if (!shouldOpen) {
        state.threadQuery = '';
        if (threadSearchInput instanceof HTMLInputElement) threadSearchInput.value = '';
      }
    }
    ...
    function queueRefreshSuggestions() {
      if (suggestionTimer) {
    ...
    async function hydrateConversationProfiles(conversations) {
      const requestId = ++conversationProfileHydrationId;
      const peers = conversations
        .filter((conversation) => conversation.peerUid && !contactDirectory.has(conversation.peerUid))
        .slice(0, 24);

      if (!peers.length) return;

      await Promise.all(peers.map((conversation) => (
        ensureContactProfile(conversation.peerUid, conversation.peerProvinceSlug || '').catch(() => null)
      )));

      if (requestId !== conversationProfileHydrationId) return;

      state.conversations = state.conversations.map((conversation) => {
        const profile = contactDirectory.get(conversation.peerUid);
        if (!profile) return conversation;
        return {
          ...conversation,
          peerName: profile.displayName || conversation.peerName,
      ...
      if (state.activePeerUid) {
        const activeProfile = contactDirectory.get(state.activePeerUid);
        if (activeProfile) {
          state.activePeerProfile = activeProfile;
          state.activePeerName = activeProfile.displayName || state.activePeerName;
    ...
    async function applyConversationList(conversations) {
      ...
      if (state.activePeerUid && !nextConversations.some((conversation) => conversation.peerUid === state.activePeerUid)) {
        const profile = contactDirectory.get(state.activePeerUid) || state.activePeerProfile || null;
    ...
    function renderThreadMessages(messages) {
      ...
      const threadQuery = state.threadQuery.trim().toLowerCase();
      const visibleMessages = threadQuery
        ? messages.filter((message) => (
          `${message.text || ''} ${message.fromName || ''} ${message.toName || ''}`.toLowerCase().includes(threadQuery)
        ))
        : messages;
      const newestMessageId = messages.length ? String(messages[messages.length - 1].id || '') : '';
      const shouldForceScroll = !threadQuery && (newestMessageId !== state.lastRenderedMessageId || !state.lastRenderedMessageId);
    ...
    async function refreshMessages() {
      if (!state.activePeerUid) {
        ...
        refreshComposerState();
        updateJobActionButton({ force: true }).catch(() => {});
        return;
      }

      const currentPeerUid = state.activePeerUid;
      const cachedProfile = contactDirectory.get(currentPeerUid);
      if (cachedProfile) {
        state.activePeerProfile = cachedProfile;
        state.activePeerName = cachedProfile.displayName || state.activePeerName;
        state.activePeerProvince = cachedProfile.provinceSlug || state.activePeerProvince;
      }
      ...
      ensureContactProfile(currentPeerUid, state.activePeerProvince).then((profile) => {
        ...
        state.activePeerName = profile.displayName || state.activePeerName;
      ...
      if (typeof authHelper.subscribeMessagesWithUser === 'function') {
        unsubscribeMessages = await authHelper.subscribeMessagesWithUser(state.activePeerUid, (messages) => {
          ...
          if (hasUnreadIncoming) {
            authHelper.markConversationViewed?.(state.activePeerUid, messages).catch(() => {});
            window.dispatchEvent(new CustomEvent('worklinkup-messages-badges-refresh'));
          }
        });
      } else {
    ...
    async function updateJobActionButton(options = {}) {
      try {
        const host = await ensureJobActionHost();
        host.innerHTML = '';
        activeJobPeerUid = state.activePeerUid || '';
      } catch (error) {
        // ignore
      }
    }
    ...
    focusSearchBtn?.addEventListener('click', () => {
      ...
      const isOpen = threadSearchShell instanceof HTMLElement && !threadSearchShell.hidden;
      setThreadSearchOpen(!isOpen);
      if (!isOpen && threadSearchInput instanceof HTMLInputElement) {
        window.requestAnimationFrame(() => threadSearchInput.focus());
      } else {
        renderThreadMessages(state.threadMessages);
      }
    });

    threadSearchInput?.addEventListener('input', (event) => {
      state.threadQuery = event.target.value.trim();
      renderThreadMessages(state.threadMessages);
    });
    ...
    if (shouldRefreshAfterAuthSession) {
      authSessionReady.then(() => refreshInboxAfterAuthSync()).catch(() => {});
    }
  ...
  async function renderClientProfilePage() {
    ...
    if (!authHelper) {
      page.innerHTML = `
        <section class="client-profile-auth-card">
          <h1>Could not load your profile tools.</h1>
    ...
    if (typeof authHelper.waitForAuthSession === 'function') {
      await Promise.race([
        authHelper.waitForAuthSession(account.uid, 5000).catch(() => null),
        new Promise((resolve) => window.setTimeout(resolve, 700))
      ]);
    }

    const [userDocResult, clientProfileResult] = await Promise.allSettled([
      typeof authHelper.getUserDocument === 'function' ? authHelper.getUserDocument(account.uid) : Promise.resolve(null),
      typeof authHelper.getClientProfileByUid === 'function' ? authHelper.getClientProfileByUid(account.uid) : Promise.resolve(null)
    ]);
    const userDoc = userDocResult.status === 'fulfilled' ? userDocResult.value || {} : {};
    const clientProfile = clientProfileResult.status === 'fulfilled' ? clientProfileResult.value || {} : {};
    const userRole = String(userDoc?.userRole || account.userRole || '').trim();
    const isCompletedProvider = userRole === 'provider' && Boolean(userDoc?.providerProfileComplete || account.providerProfileComplete);

    if (isCompletedProvider) {
      const redirectParams = new URLSearchParams();
      redirectParams.set('uid', account.uid);
      if (userDoc?.providerProvinceSlug || account.providerProvinceSlug) {
        redirectParams.set('province', userDoc?.providerProvinceSlug || account.providerProvinceSlug);
      }
      window.location.replace(`${base}pages/provider-profile.html?${redirectParams.toString()}`);
      return;
    }
    ...
    const profile = {
      ...
      address: clientProfile.address || userDoc.address || '',
      bio: clientProfile.bio || userDoc.bio || '',
    ...
    const locationLabel = [profile.address, profile.city].filter(Boolean).join(', ') || 'Location not shared';
    const phoneHref = profile.phone ? `tel:${profile.phone.replace(/[^+\d]/g, '')}` : '#';

    page.innerHTML = `
      <section class="provider-profile-hero client-profile-provider-style">
        <div class="provider-profile-banner" style="background-image: linear-gradient(180deg, rgba(13, 28, 56, 0.10) 0%, rgba(13, 28, 56, 0.66) 100%), url('${escapeHtml(bannerSrc)}');">
          <button type="button" class="provider-profile-back" data-client-profile-back aria-label="Go back">
            <i class="fa-solid fa-arrow-left"></i>
          </button>
          <div class="provider-profile-avatar-ring">
            <img src="${escapeHtml(avatarSrc)}" alt="${escapeHtml(profile.displayName)}" />
          </div>

          <div class="provider-profile-banner-copy">
            <h1>${escapeHtml(profile.displayName)}</h1>
            <div class="provider-profile-banner-meta">
              <div class="provider-profile-handle">${escapeHtml(handle)}</div>
              <a href="${escapeHtml(phoneHref)}" class="provider-profile-phone">
                <i class="fa-solid fa-phone"></i>
                <span>${escapeHtml(profile.phone || 'Phone not shared')}</span>
              </a>
            </div>
          </div>
        </div>
      ...
      <section class="provider-profile-gallery-head">
        <div class="provider-profile-gallery-tabs">
          <button type="button" class="provider-profile-gallery-tab is-active" data-client-profile-tab="details">
            <span>Profile Details</span>
          </button>
          <button type="button" class="provider-profile-gallery-tab" data-client-profile-tab="edit">
            <span>Edit Profile</span>
          </button>
        </div>
      </section>

      <section class="provider-profile-gallery client-profile-gallery is-active-gallery" data-client-profile-panel="details">
        <article class="provider-gallery-card client-profile-info-card">
          <div class="provider-gallery-card-copy">
            <strong>Email</strong>
            <p>${escapeHtml(profile.email || 'Not set')}</p>
          </div>
        </article>
        <article class="provider-gallery-card client-profile-info-card">
          <div class="provider-gallery-card-copy">
            <strong>Phone</strong>
            <p>${escapeHtml(profile.phone || 'Not set')}</p>
          </div>
        </article>
        <article class="provider-gallery-card client-profile-info-card">
          <div class="provider-gallery-card-copy">
      ...
      <section class="provider-profile-gallery client-profile-gallery" data-client-profile-panel="edit" hidden>
        <article class="provider-gallery-card client-profile-edit-panel">
          <form class="client-profile-form" data-client-profile-form>
            <div class="client-profile-card-head">
              <span>Edit client profile</span>
              <strong>This updates your client profile only. Provider setup uses the Become Provider button.</strong>
            </div>
            <label>
              <span>Name</span>
            ...
            <label>
              ...
              <input type="text" name="address" value="${escapeHtml(profile.address)}" />
            </label>
            <label>
              <span>Short bio</span>
              <textarea name="bio" rows="4">${escapeHtml(profile.bio)}</textarea>
            </label>
            <label class="client-profile-upload">
              <i class="fa-regular fa-image"></i>
              <span>Change profile photo</span>
              <input type="file" accept="image/*" data-client-profile-image />
            </label>
            <button type="submit" class="provider-profile-action client-profile-save-btn" data-client-profile-save>
    ...
    const form = page.querySelector('[data-client-profile-form]');
    const imageInput = page.querySelector('[data-client-profile-image]');
    const saveBtn = page.querySelector('[data-client-profile-save]');
    const becomeProviderBtns = page.querySelectorAll('[data-client-become-provider]');
    ...
    imageInput?.addEventListener('change', async () => {
      ...
      if (!file) return;
      try {
        pendingProfileImageData = await readImageAsBase64(file, {
          maxWidth: 720,
          maxHeight: 720,
          quality: 0.84,
          outputType: 'image/avif'
    ...
    form?.addEventListener('submit', async (event) => {
      ...
      if (saveBtn instanceof HTMLButtonElement) setButtonLoading(saveBtn, true);
      try {
        await authHelper.saveClientProfile(payload);
        showSuccessToast('Client profile saved');
        window.setTimeout(() => window.location.reload(), 650);
      } catch (error) {
        window.alert(error.message || 'Could not save your client profile.');
      } finally {
        if (saveBtn instanceof HTMLButtonElement) setButtonLoading(saveBtn, false);
      }
    });

    function setClientProfileTab(name = 'details') {
      const nextName = String(name || 'details').trim();
      page.querySelectorAll('[data-client-profile-tab]').forEach((tab) => {
        const isActive = tab.getAttribute('data-client-profile-tab') === nextName;
        tab.classList.toggle('is-active', isActive);
        tab.setAttribute('aria-selected', String(isActive));
      });
      page.querySelectorAll('[data-client-profile-panel]').forEach((panel) => {
        const isActive = panel.getAttribute('data-client-profile-panel') === nextName;
        panel.hidden = !isActive;
        panel.classList.toggle('is-active-gallery', isActive);
      });
    }

    page.querySelectorAll('[data-client-profile-tab]').forEach((tab) => {
      tab.addEventListener('click', () => setClientProfileTab(tab.getAttribute('data-client-profile-tab') || 'details'));
    });

    page.querySelector('[data-client-edit-tab-trigger]')?.addEventListener('click', () => {
      setClientProfileTab('edit');
      page.querySelector('[data-client-profile-panel="edit"]')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
    });

    page.querySelector('[data-client-profile-back]')?.addEventListener('click', () => {
      if (window.history.length > 1) {
        window.history.back();
        return;
      }
    ...
    becomeProviderBtns.forEach((button) => {
      button.addEventListener('click', openBecomeProvider);
    });
  }
      ...
      if (step === 'dashboard') {
        const getProfileUrl = () => {
          if (String(userDoc?.userRole || '').trim() === 'provider') {
            const redirectParams = new URLSearchParams();
            redirectParams.set('uid', account.uid);
            if (providerProfile?.provinceSlug || userDoc?.providerProvinceSlug) {
              redirectParams.set('province', providerProfile?.provinceSlug || userDoc?.providerProvinceSlug);
            }
            return `${getBase()}pages/provider-profile.html?${redirectParams.toString()}`;
          }
          return `${getBase()}pages/client-profile.html`;
        };
