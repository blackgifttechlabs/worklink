(function searchIntelligenceBootstrap(root, factory) {
  ...
  const SEARCH_ALIASES = [
    ['maid', 'Domestic Cleaner / Maid / Housekeeper', 'Cleaning, Sanitation & Waste Management'],
    ['house maid', 'Domestic Cleaner / Maid / Housekeeper', 'Cleaning, Sanitation & Waste Management'],
    ['cleaner', 'Domestic Cleaner / Maid / Housekeeper', 'Cleaning, Sanitation & Waste Management'],
    ['house cleaning', 'Domestic Cleaner / Maid / Housekeeper', 'Cleaning, Sanitation & Waste Management'],
    ['capender', 'Carpenter & Roofer / Thatcher', 'Construction, Trades & Architecture'],
    ['capenter', 'Carpenter & Roofer / Thatcher', 'Construction, Trades & Architecture'],
    ['carpender', 'Carpenter & Roofer / Thatcher', 'Construction, Trades & Architecture'],
    ['wood work', 'Carpenter & Roofer / Thatcher', 'Construction, Trades & Architecture'],
    ['woodwork', 'Carpenter & Roofer / Thatcher', 'Construction, Trades & Architecture'],
    ['garden', 'Landscaper & Tree Feller', 'Agriculture, Forestry & Water'],
    ['gardens', 'Landscaper & Tree Feller', 'Agriculture, Forestry & Water'],
    ['gardener', 'Landscaper & Tree Feller', 'Agriculture, Forestry & Water'],
    ['lawn', 'Landscaper & Tree Feller', 'Agriculture, Forestry & Water'],
    ['mowing', 'Landscaper & Tree Feller', 'Agriculture, Forestry & Water'],
    ['fix pipes', 'Plumber', 'Plumber'],
    ['blocked drain', 'Plumber', 'Plumber'],
    ['geyser', 'Plumber', 'Plumber'],
    ['wiring', 'Electronics Technician', 'Installations, Energy & Appliance Repair'],
    ['solar', 'Solar Panel & Inverter Installer', 'Installations, Energy & Appliance Repair'],
    ['power backup', 'Battery Technician', 'Battery Technician'],
    ['build wall', 'Builder / Bricklayer', 'Construction, Trades & Architecture'],
    ['web site', 'Software Programmer / App Developer', 'Digital, IT & Tech Services'],
    ['website', 'Web Designer', 'Digital, IT & Tech Services'],
    ['app', 'Software Programmer / App Developer', 'Digital, IT & Tech Services'],
    ['computer repair', 'IT Support Specialist / Network Engineer', 'Digital, IT & Tech Services'],
    ['phone repair', 'Mobile Phone & Laptop Repair Technician', 'Digital, IT & Tech Services'],
    ['make up', 'Nail Technician', 'Nail Technician'],
    ['makeup', 'Nail Technician', 'Nail Technician'],
    ['nails', 'Nail Technician', 'Nail Technician'],
    ['moving', 'Furniture Remover', 'Logistics, Freight & Courier Services'],
    ['truck', 'Loader/Unloader (Marovha / Makorokoza)', 'Logistics, Freight & Courier Services'],
    ['taxi', 'Taxi Driver (Formal) & Informal Taxi Driver (Mushikashika)', 'Automotive & Transport Services'],
    ['wedding photos', 'Photographer & Videographer', 'Creative, Media & Advertising'],
    ['photos', 'Photographer & Videographer', 'Creative, Media & Advertising'],
    ['maths', 'Tutor / Extra Lessons Teacher', 'Education, Training & Academic Services'],
    ['math', 'Tutor / Extra Lessons Teacher', 'Education, Training & Academic Services'],
    ['nanny', 'Babysitter / Nanny', 'Care & Animal Services'],
    ['security camera', 'DStv/Satellite Installer', 'DStv/Satellite Installer'],
    ['cctv', 'DStv/Satellite Installer', 'DStv/Satellite Installer'],
    ['water tank', 'Water Engineer', 'Water Engineer'],
    ['borehole', 'Borehole Driller & Pump Installer', 'Agriculture, Forestry & Water'],
    ['catering', 'Caterer', 'Caterer'],
    ['cake', 'Caterer', 'Caterer']
  ];
  ...
  function buildCatalogEntries(catalog = getCatalog(), locations = getLocations()) {
    ...
    SEARCH_ALIASES.forEach(([alias, service, category]) => {
      const catalogCategory = catalog.find((item) => String(item.label || '').trim().toLowerCase() === String(service || '').trim().toLowerCase()
        || (Array.isArray(item.subservices) ? item.subservices : []).some((subservice) => String(subservice || '').trim().toLowerCase() === String(service || '').trim().toLowerCase()));
      const resolvedCategory = catalogCategory?.label || category;
      entries.push({
        kind: 'alias',
        title: service,
        query: service,
        ...
        terms: [alias, service, resolvedCategory]
      });
    });
